      ...
      if (html[pos] === '<') {
        ...
        if (html[pos + 1] === '/') {
          ...
          if (tagName) {
            ...
            if (preserveDepth > 0 && result.length) {
              // Keep the close tag on the SAME line as the preserved content — a
              // newline here renders inside pre/code (white-space:pre/break-spaces).
              result[result.length - 1] += fullTag;
        ...
        if (preserveDepth > 0 && result.length) {
          result[result.length - 1] += fullOpenTag; // nested tag inside preserved content — stay inline
        } else {
        ...
        // Text content
        if (preserveDepth > 0) {
          // In preserve mode, output text as-is, appended inline (no newline)
          let textEnd = html.indexOf('<', pos);
          if (textEnd === -1) textEnd = len;
          const raw = html.substring(pos, textEnd);
          if (result.length) result[result.length - 1] += raw; else result.push(raw);
          pos = textEnd;
        } else {
          let textEnd = html.indexOf('<', pos);
          if (textEnd === -1) textEnd = len;
